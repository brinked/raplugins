...
class ECP_Article_Health {
    ...
    public function add_dashboard_menu() {
        add_menu_page(
            __('Article Health', 'enhanced-content-plugin'),
            __('Article Health', 'enhanced-content-plugin'),
            'edit_posts',
            'map-article-health',
    ...
    public function enqueue_dashboard_assets($hook) {
        ...
        wp_enqueue_style(
            ...
            ECP_PLUGIN_URL . 'admin/css/article-health-styles.css',
            array(),
            ECP_VERSION
        );
        ...
        wp_enqueue_script(
            ...
            ECP_PLUGIN_URL . 'admin/js/article-health-scripts.js',
    ...
    public function add_health_columns($columns) {
        ...
        foreach ($columns as $key => $value) {
            ...
            // Insert after title
            if ($key === 'title') {
                $new_columns['map_word_count'] = '<span class="dashicons dashicons-editor-paste-word" title="' . esc_attr__('Word Count', 'enhanced-content-plugin') . '"></span>';
                $new_columns['map_citations'] = '<span class="dashicons dashicons-admin-links" title="' . esc_attr__('Citations', 'enhanced-content-plugin') . '"></span>';
                $new_columns['map_freshness'] = '<span class="dashicons dashicons-calendar-alt" title="' . esc_attr__('Days Since Update', 'enhanced-content-plugin') . '"></span>';
            }
        }
    ...
    public function render_health_columns($column, $post_id) {
        switch ($column) {
            case 'map_word_count':
                ...
                echo '<span class="' . esc_attr($class) . '" title="' . esc_attr(sprintf(__('%d words', 'enhanced-content-plugin'), $word_count)) . '">';
            ...
            case 'map_citations':
                ...
                echo '<span class="' . esc_attr($class) . '" title="' . esc_attr(sprintf(__('%d citations', 'enhanced-content-plugin'), $citations)) . '">';
            ...
            case 'map_freshness':
                ...
                echo '<span class="' . esc_attr($class) . '" title="' . esc_attr(sprintf(__('%d days since last update', 'enhanced-content-plugin'), $days)) . '">';
    ...
    public function add_health_indicators($actions, $post) {
        ...
        if ($word_count < 500) {
            $issues[] = sprintf(__('Low word count (%d)', 'enhanced-content-plugin'), $word_count);
        }

        $citations = $this->get_citation_count($post->ID);
        if ($citations === 0) {
            $issues[] = __('No citations', 'enhanced-content-plugin');
        }

        $days = $this->get_days_since_update($post->ID);
        if ($days > 365) {
            $issues[] = sprintf(__('Not updated in %d days', 'enhanced-content-plugin'), $days);
        }
    ...
    public function show_health_notices() {
        ...
        if ($word_count < 500) {
            $issues[] = sprintf(
                __('This article has only %d words. Consider adding more content for better SEO.', 'enhanced-content-plugin'),
                $word_count
            );
        }
        ...
        $citations = $this->get_citation_count($post->ID);
        if ($citations === 0) {
            $issues[] = __('This article has no citations. Consider adding sources to improve credibility.', 'enhanced-content-plugin');
        }

        $days = $this->get_days_since_update($post->ID);
        if ($days > 365) {
            $issues[] = sprintf(
                __('This article hasn\'t been updated in %d days. Consider reviewing and refreshing the content.', 'enhanced-content-plugin'),
                $days
            );
        }
        ...
        if (!empty($issues)) {
            ...
            echo '<p><strong>' . esc_html__('Article Health Issues:', 'enhanced-content-plugin') . '</strong></p>';
        ...
        <div class="wrap map-article-health-dashboard">
            ...
            <div class="map-health-summary">
                <div class="map-health-card map-health-card-total">
                    ...
                    <span class="map-health-card-label"><?php _e('Total Articles', 'enhanced-content-plugin'); ?></span>
                </div>
                ...
                <div class="map-health-card map-health-card-warning">
                    ...
                    <span class="map-health-card-label"><?php _e('Low Word Count', 'enhanced-content-plugin'); ?></span>
                    <span class="map-health-card-desc"><?php _e('Under 500 words', 'enhanced-content-plugin'); ?></span>
                </div>

                <div class="map-health-card map-health-card-warning">
                    <span class="map-health-card-number"><?php echo esc_html($stats['no_citations']); ?></span>
                    <span class="map-health-card-label"><?php _e('No Citations', 'enhanced-content-plugin'); ?></span>
                    <span class="map-health-card-desc"><?php _e('Missing sources', 'enhanced-content-plugin'); ?></span>
                </div>

                <div class="map-health-card map-health-card-critical">
                    <span class="map-health-card-number"><?php echo esc_html($stats['stale']); ?></span>
                    <span class="map-health-card-label"><?php _e('Stale Articles', 'enhanced-content-plugin'); ?></span>
                    <span class="map-health-card-desc"><?php _e('Not updated in 1+ year', 'enhanced-content-plugin'); ?></span>
                </div>
            </div>
            ...
            <div class="map-health-filters">
                <a href="<?php echo esc_url(add_query_arg('filter', 'all', remove_query_arg(array('paged')))); ?>"
                   class="button <?php echo $filter === 'all' ? 'button-primary' : ''; ?>">
                    <?php _e('All Articles', 'enhanced-content-plugin'); ?>
                </a>
                <a href="<?php echo esc_url(add_query_arg('filter', 'low_words', remove_query_arg(array('paged')))); ?>"
                   class="button <?php echo $filter === 'low_words' ? 'button-primary' : ''; ?>">
                    <?php _e('Low Word Count', 'enhanced-content-plugin'); ?>
                </a>
                <a href="<?php echo esc_url(add_query_arg('filter', 'no_citations', remove_query_arg(array('paged')))); ?>"
                   class="button <?php echo $filter === 'no_citations' ? 'button-primary' : ''; ?>">
                    <?php _e('No Citations', 'enhanced-content-plugin'); ?>
                </a>
                <a href="<?php echo esc_url(add_query_arg(array('filter' => 'stale', 'orderby' => 'modified', 'order' => 'ASC'), remove_query_arg(array('paged')))); ?>"
                   class="button <?php echo $filter === 'stale' ? 'button-primary' : ''; ?>">
                    <?php _e('Stale (Oldest First)', 'enhanced-content-plugin'); ?>
                </a>
            </div>

            <!-- Sort Options -->
            <div class="map-health-sort">
                <span><?php _e('Sort by:', 'enhanced-content-plugin'); ?></span>
                <a href="<?php echo esc_url(add_query_arg(array('orderby' => 'word_count', 'order' => 'ASC'))); ?>"
                   class="<?php echo $orderby === 'word_count' ? 'active' : ''; ?>">
                    <?php _e('Word Count', 'enhanced-content-plugin'); ?>
                ...
                <a href="<?php echo esc_url(add_query_arg(array('orderby' => 'citations', 'order' => 'ASC'))); ?>"
                   class="<?php echo $orderby === 'citations' ? 'active' : ''; ?>">
                    <?php _e('Citations', 'enhanced-content-plugin'); ?>
                ...
                <a href="<?php echo esc_url(add_query_arg(array('orderby' => 'modified', 'order' => 'ASC'))); ?>"
                   class="<?php echo $orderby === 'modified' ? 'active' : ''; ?>">
                    <?php _e('Last Updated', 'enhanced-content-plugin'); ?>
                ...
                <?php if ($orderby) : ?>
                    <a href="<?php echo esc_url(add_query_arg('order', $order === 'ASC' ? 'DESC' : 'ASC')); ?>"
                       class="button button-small">
                        <?php echo $order === 'ASC' ? __('↓ Descending', 'enhanced-content-plugin') : __('↑ Ascending', 'enhanced-content-plugin'); ?>
                    </a>
                <?php endif; ?>
            </div>
            ...
            <table class="wp-list-table widefat fixed striped">
                <thead>
                    <tr>
                        <th class="column-title"><?php _e('Article', 'enhanced-content-plugin'); ?></th>
                        <th class="column-words"><?php _e('Words', 'enhanced-content-plugin'); ?></th>
                        <th class="column-citations"><?php _e('Citations', 'enhanced-content-plugin'); ?></th>
                        <th class="column-updated"><?php _e('Last Updated', 'enhanced-content-plugin'); ?></th>
                        <th class="column-status"><?php _e('Health', 'enhanced-content-plugin'); ?></th>
                        <th class="column-actions"><?php _e('Actions', 'enhanced-content-plugin'); ?></th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <?php else : ?>
                        <tr>
                            <td colspan="6"><?php _e('No articles found.', 'enhanced-content-plugin'); ?></td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
            ...
            <div class="tablenav bottom">
                <div class="tablenav-pages">
                    <span class="displaying-num">
                        ...
                        printf(
                            _n('%s item', '%s items', $total_items, 'enhanced-content-plugin'),
                    ...
                    <span class="pagination-links">
                        <label for="map-per-page" class="screen-reader-text"><?php _e('Items per page', 'enhanced-content-plugin'); ?></label>
                        <select id="map-per-page" onchange="window.location.href=this.value;">
                            ...
                            foreach ($per_page_options as $option) :
                                $label = $option === -1 ? __('All', 'enhanced-content-plugin') : $option;
                                $url = add_query_arg('per_page', $option, $base_url);
                            ?>
                                <option value="<?php echo esc_url($url); ?>" <?php selected($per_page, $option); ?>>
                                    <?php echo esc_html($label); ?> <?php _e('per page', 'enhanced-content-plugin'); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </span>
    ...
    public function ajax_recalculate_health() {
        ...
        wp_send_json_success(array(
            ...
            'total' => $result['total'],
            'done' => $done,
            'message' => sprintf(__('Recalculated metrics for %d articles.', 'enhanced-content-plugin'), $offset + $result['processed'])
        ));
    }
