...
class ECP_Ajax {
    ...
    /**
     * A long AI call must survive the gateway giving up on the browser:
     * the work continues, the result is stored, and the page finds it by
     * polling plan_status. Raising the time limit is best-effort — hosts
     * that forbid it simply keep their own ceiling.
     */
    private function prepare_long_job() {
    ...
    /**
     * Lightweight progress probe for the long builds. A browser whose
     * request was cut by a gateway timeout polls this to learn that the
     * work finished anyway: the map's newest row, a topic's brief, or a
     * brief's draft. Read-only.
     */
    public function plan_status() {
        ...
        if ('map' === $what) {
            $seed = isset($_POST['key']) ? sanitize_text_field(wp_unslash($_POST['key'])) : '';
            $row = $wpdb->get_row($wpdb->prepare(
                'SELECT COUNT(*) AS total, MAX(updated_at) AS latest FROM ' . ECP_DB::topics_table() . ' WHERE seed = %s',
                $seed
            ), ARRAY_A);
            $count = (int) $row['total'];
            $latest = $row['latest'] ? (int) strtotime($row['latest']) : 0;
        } elseif ('brief' === $what) {
            $topic_id = isset($_POST['key']) ? absint($_POST['key']) : 0;
            $row = $wpdb->get_row($wpdb->prepare(
                'SELECT id, updated_at FROM ' . ECP_DB::briefs_table() . ' WHERE topic_id = %d',
                $topic_id
            ), ARRAY_A);
            $count = $row ? 1 : 0;
            $latest = $row && $row['updated_at'] ? (int) strtotime($row['updated_at']) : 0;
        } elseif ('draft' === $what) {
            $brief_id = isset($_POST['key']) ? absint($_POST['key']) : 0;
            $row = $wpdb->get_row($wpdb->prepare(
                'SELECT draft_post_id, drafted_at FROM ' . ECP_DB::briefs_table() . ' WHERE id = %d',
                $brief_id
            ), ARRAY_A);
            $count = $row && (int) $row['draft_post_id'] ? 1 : 0;
            $latest = $row && $row['drafted_at'] ? (int) strtotime($row['drafted_at']) : 0;
        } else {
            wp_send_json_error(array('message' => __('Unknown status probe.', 'enhanced-content-plugin')));
        }

        wp_send_json_success(array('count' => $count, 'latest' => $latest));
    }
