...
class MAP_Frontend_Display {
    ...
    public function maybe_disable_theme_author_date() {
        ...
        $hide_theme_author = MAP_Settings::get_setting('hide_theme_author', 0);
    ...
    public function return_empty_for_theme($value) {
        ...
        if (is_admin() || !is_singular(MAP_Settings::get_enabled_post_types()) || !in_the_loop()) {
            return $value;
        }
        return '';
    }
     ...
     *   your-theme/multi-author-plugin/{template-name}.php
     * Developers can also reroute paths via the 'map_template_path' filter.
     */
    public static function locate_template($template) {
        $theme_template = locate_template('multi-author-plugin/' . $template);
        $path = $theme_template ? $theme_template : MAP_PLUGIN_DIR . 'templates/' . $template;
    ...
    public function add_contributors_to_content($content) {
        ...
        if ('manual' === MAP_Settings::get_setting('contributors_placement', 'auto')) {
            return $content;
        }
    ...
    public function add_sources_to_content($content) {
        ...
        if ('manual' === MAP_Settings::get_setting('sources_placement', 'auto')) {
            return $content;
        }
    ...
    public function add_author_archive_credits($description) {
        if (!is_author() || !MAP_Settings::get_setting('author_archive_credits', 0)) {
            return $description;
        }
        ...
        if ($counts['reviewed']) {
            ...
            $parts[] = sprintf(_n('Reviewed %s article', 'Reviewed %s articles', $counts['reviewed'], 'multi-author-plugin'), number_format_i18n($counts['reviewed']));
        }
        if ($counts['fact_checked']) {
            /* translators: %s: number of articles */
            $parts[] = sprintf(_n('Fact-checked %s article', 'Fact-checked %s articles', $counts['fact_checked'], 'multi-author-plugin'), number_format_i18n($counts['fact_checked']));
        }
