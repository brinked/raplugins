...
class MAP_Integrations {
    ...
    public function maybe_show_seo_notice() {
        ...
        if (MAP_Settings::get_setting('disable_article_schema', 0)) {
            return;
        }
        ...
        if (!$screen || !in_array($screen->id, array('dashboard', 'plugins', 'settings_page_multi-author-settings'), true)) {
            return;
        }

        $dismiss_url = wp_nonce_url(add_query_arg('map_dismiss_seo_notice', '1'), 'map_dismiss_seo_notice');
        $settings_url = admin_url('options-general.php?page=multi-author-settings');
        ?>
        <div class="notice notice-warning">
            <p>
                <strong><?php esc_html_e('Multi-Author Contributor Plugin:', 'multi-author-plugin'); ?></strong>
                <?php
                printf(
                    /* translators: %s: SEO plugin name */
                    esc_html__('%s is active and may already output Article schema. Emitting two Article schemas on the same page can cause Search Console conflicts. You can disable this plugin\'s Article schema under Display Options.', 'multi-author-plugin'),
                    esc_html($seo_plugin)
                );
                ?>
            </p>
            <p>
                <a href="<?php echo esc_url($settings_url); ?>" class="button button-primary"><?php esc_html_e('Review Settings', 'multi-author-plugin'); ?></a>
                <a href="<?php echo esc_url($dismiss_url); ?>" class="button"><?php esc_html_e('Dismiss — keep both', 'multi-author-plugin'); ?></a>
            </p>
        </div>
        <?php
    }
    ...
    public function register_privacy_exporter($exporters) {
        $exporters['multi-author-plugin'] = array(
            'exporter_friendly_name' => __('Multi-Author Contributor Plugin', 'multi-author-plugin'),
    ...
    public function export_personal_data($email_address, $page = 1) {
        ...
        if ($user) {
            $fields = array(
                '_user_short_bio' => __('Short Bio', 'multi-author-plugin'),
                '_user_editorial_process_link' => __('Editorial Process Link', 'multi-author-plugin'),
                '_contact_email' => __('Public Contact Email', 'multi-author-plugin'),
                '_website_url' => __('Personal Website', 'multi-author-plugin'),
                'job_title' => __('Job Title', 'multi-author-plugin'),
                'twitter' => __('Twitter/X Profile', 'multi-author-plugin'),
                'linkedin' => __('LinkedIn Profile', 'multi-author-plugin'),
                'facebook' => __('Facebook Profile', 'multi-author-plugin'),
                'instagram' => __('Instagram Profile', 'multi-author-plugin'),
                'youtube' => __('YouTube Channel', 'multi-author-plugin'),
            );
            ...
            if (!empty($data)) {
                $export_items[] = array(
                    ...
                    'group_label' => __('Contributor Profile', 'multi-author-plugin'),
    ...
    public function register_privacy_eraser($erasers) {
        $erasers['multi-author-plugin'] = array(
            'eraser_friendly_name' => __('Multi-Author Contributor Plugin', 'multi-author-plugin'),
    ...
    public function register_site_health_tests($tests) {
        $tests['direct']['map_orphaned_contributors'] = array(
            'label' => __('Multi-Author: contributor data is valid', 'multi-author-plugin'),
            'test' => array($this, 'site_health_orphaned_contributors'),
        );
        $tests['direct']['map_schema_conflict'] = array(
            'label' => __('Multi-Author: no duplicate Article schema', 'multi-author-plugin'),
    ...
    public function site_health_orphaned_contributors() {
        ...
        $result = array(
            'label' => __('Contributor data references valid users', 'multi-author-plugin'),
            'status' => 'good',
            'badge' => array('label' => __('Content', 'multi-author-plugin'), 'color' => 'blue'),
            'description' => '<p>' . esc_html__('All contributor assignments checked point at existing user accounts.', 'multi-author-plugin') . '</p>',
    ...
    public function site_health_schema_conflict() {
        $result = array(
            'label' => __('Only one plugin outputs Article schema', 'multi-author-plugin'),
            'status' => 'good',
            'badge' => array('label' => __('SEO', 'multi-author-plugin'), 'color' => 'blue'),
            'description' => '<p>' . esc_html__('No duplicate Article structured data detected.', 'multi-author-plugin') . '</p>',
            'test' => 'map_schema_conflict',
        );

        $seo_plugin = self::seo_plugin_active();

        if ($seo_plugin && !MAP_Settings::get_setting('disable_article_schema', 0)) {
            $result['status'] = 'recommended';
            $result['label'] = __('Two plugins may output Article schema', 'multi-author-plugin');
            $result['description'] = '<p>' . sprintf(
                /* translators: %s: SEO plugin name */
                esc_html__('%s is active and may output Article schema alongside this plugin\'s. Consider disabling one of them (Settings > Multi-Author > Display Options > Disable Article Schema).', 'multi-author-plugin'),
                esc_html($seo_plugin)
            ) . '</p>';
            $result['actions'] = sprintf(
                '<a href="%s">%s</a>',
                esc_url(admin_url('options-general.php?page=multi-author-settings')),
                esc_html__('Open Multi-Author settings', 'multi-author-plugin')
            );
        }
