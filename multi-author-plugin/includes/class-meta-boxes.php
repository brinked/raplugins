...
class MAP_Meta_Boxes {
    ...
    public function add_meta_boxes() {
        $post_types = MAP_Settings::get_enabled_post_types();
        ...
        add_meta_box(
            'map_ai_disclaimer',
            __('AI Disclosure Badge', 'multi-author-plugin'),
        ...
        add_meta_box(
            'map_fact_check',
            __('Fact Check & Corrections', 'multi-author-plugin'),
        ...
        <div class="map-fact-check-wrapper">
            <div class="map-fact-check-date-field">
                <label for="map_fact_checked_date">
                    <strong><?php _e('Last fact-checked on', 'multi-author-plugin'); ?></strong>
                </label>
                <input type="date"
                ...
                <p class="description">
                    <?php _e('Shown in the article byline, separate from the last-modified date. Leave empty to hide.', 'multi-author-plugin'); ?>
                </p>
            </div>

            <div class="map-corrections-header">
                <h4><?php _e('Corrections Log', 'multi-author-plugin'); ?></h4>
                <p class="description"><?php _e('Log corrections transparently. Entries are displayed in a "Corrections & Updates" section at the end of the article.', 'multi-author-plugin'); ?></p>
            </div>
            ...
            <button type="button" class="button" id="map-add-correction">
                <?php _e('+ Add Correction', 'multi-author-plugin'); ?>
            </button>
        </div>
    ...
    private function render_correction_item($index, $correction) {
        ...
        <div class="map-correction-item">
            ...
            <textarea name="map_corrections[<?php echo esc_attr($index); ?>][text]"
                      placeholder="<?php esc_attr_e('Describe the correction, e.g. "Corrected the survey sample size in section 2."', 'multi-author-plugin'); ?>"
                      class="map-correction-text widefat"
                      rows="2"><?php echo esc_textarea($text); ?></textarea>
            <button type="button" class="button-link map-remove-correction" aria-label="<?php esc_attr_e('Remove', 'multi-author-plugin'); ?>">
    ...
    public function render_contributors_meta_box($post) {
        ...
        $expert_verified_enabled = MAP_Settings::get_setting('expert_verified_enabled', 0);
        ...
        <div class="map-contributors-wrapper">
            ...
            <div class="map-contributor-section map-expert-verified-section" style="background: #f0f7ff; padding: 15px; border-radius: 4px; margin-bottom: 20px; border-left: 4px solid #0073aa;">
                <label style="display: flex; align-items: center; gap: 10px; cursor: pointer;">
                    ...
                    <strong><?php _e('Show "Expert Verified" badge on this post', 'multi-author-plugin'); ?></strong>
                </label>
                <p class="description" style="margin: 8px 0 0 26px;">
                    <?php _e('When enabled, the Expert Verified badge will appear in the author byline for this post.', 'multi-author-plugin'); ?>
                </p>
            </div>
            <?php endif; ?>
            ...
            <div class="map-contributor-section">
                ...
                <div class="map-contributor-list" id="map-authors-list" data-type="authors" data-empty-message="<?php esc_attr_e('No co-authors added yet.', 'multi-author-plugin'); ?>">
                ...
                <div class="map-process-link-toggle" style="margin-top: 10px; padding: 8px 12px; background: #f9f9f9; border-radius: 4px;">
                    <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                        ...
                        <span><?php _e('Show Editorial Process link in author popups', 'multi-author-plugin'); ?></span>
                    </label>
                </div>
                <?php endif; ?>
            </div>
            ...
            <div class="map-contributor-section">
                ...
                <div class="map-contributor-list" id="map-reviewers-list" data-type="reviewers" data-empty-message="<?php esc_attr_e('No reviewers added yet.', 'multi-author-plugin'); ?>">
                ...
                <div class="map-process-link-toggle" style="margin-top: 10px; padding: 8px 12px; background: #f9f9f9; border-radius: 4px;">
                    <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                        ...
                        <span><?php _e('Show Review Process link in reviewer popups', 'multi-author-plugin'); ?></span>
                    </label>
                </div>
                <?php endif; ?>
            </div>
            ...
            <div class="map-contributor-section">
                ...
                <div class="map-contributor-list" id="map-fact-checkers-list" data-type="fact_checkers" data-empty-message="<?php esc_attr_e('No fact checkers added yet.', 'multi-author-plugin'); ?>">
                ...
                <div class="map-process-link-toggle" style="margin-top: 10px; padding: 8px 12px; background: #f9f9f9; border-radius: 4px;">
                    <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                        ...
                        <span><?php _e('Show Fact-Check Process link in fact-checker popups', 'multi-author-plugin'); ?></span>
                    </label>
                </div>
                <?php endif; ?>
            </div>
        ...
        <div id="map-user-search-modal" class="map-modal" style="display: none;">
            <div class="map-modal-content">
                ...
                <input type="text" id="map-user-search-input" placeholder="<?php esc_attr_e('Search by name or email...', 'multi-author-plugin'); ?>" />
    ...
    private function render_single_contributor($user, $type) {
        ...
        <div class="map-contributor-item" data-user-id="<?php echo esc_attr($user->ID); ?>">
            ...
            <button type="button" class="button-link map-remove-contributor" aria-label="<?php esc_attr_e('Remove', 'multi-author-plugin'); ?>">
    ...
    private function render_source_item($index, $source) {
        ...
        <div class="map-source-item">
            <span class="map-source-drag-handle" title="<?php esc_attr_e('Drag to reorder', 'multi-author-plugin'); ?>">⋮⋮</span>
            ...
            <div class="map-source-fields">
                <input type="url"
                       ...
                       placeholder="<?php esc_attr_e('https://example.com/source', 'multi-author-plugin'); ?>"
                       value="<?php echo esc_attr($url); ?>"
                       class="map-source-url widefat" />
                <input type="text"
                       name="map_sources[<?php echo esc_attr($index); ?>][label]"
                       placeholder="<?php esc_attr_e('Link text (optional - displays instead of raw URL)', 'multi-author-plugin'); ?>"
                       value="<?php echo esc_attr($label); ?>"
                       class="map-source-label widefat" />
                <textarea name="map_sources[<?php echo esc_attr($index); ?>][description]"
                          placeholder="<?php esc_attr_e('Additional details or description (optional - not linked)', 'multi-author-plugin'); ?>"
                          class="map-source-description widefat"
                          rows="2"><?php echo esc_textarea($description); ?></textarea>
            </div>
            <button type="button" class="button-link map-remove-source" aria-label="<?php esc_attr_e('Remove', 'multi-author-plugin'); ?>">
                <span class="dashicons dashicons-trash"></span>
            </button>
        </div>
        <?php
    ...
    public function render_ai_disclaimer_meta_box($post) {
        ...
        $predefined_uses = array(
            'research' => __('Research & Information Gathering', 'multi-author-plugin'),
            'writing_assistance' => __('Writing Assistance', 'multi-author-plugin'),
            'editing' => __('Editing & Proofreading', 'multi-author-plugin'),
            'spellcheck' => __('Spell Checking', 'multi-author-plugin'),
            'grammar' => __('Grammar Checking', 'multi-author-plugin'),
            'formatting' => __('Formatting & Structure', 'multi-author-plugin'),
            'summarization' => __('Summarization', 'multi-author-plugin'),
            'translation' => __('Translation', 'multi-author-plugin'),
            'image_creation' => __('Image Creation/Generation', 'multi-author-plugin'),
            'image_editing' => __('Image Editing/Enhancement', 'multi-author-plugin'),
            'video_creation' => __('Video Creation/Generation', 'multi-author-plugin'),
            'video_editing' => __('Video Editing', 'multi-author-plugin'),
            'audio_creation' => __('Audio/Voice Generation', 'multi-author-plugin'),
            'transcription' => __('Transcription', 'multi-author-plugin'),
            'data_analysis' => __('Data Analysis', 'multi-author-plugin'),
            'code_generation' => __('Code Generation', 'multi-author-plugin'),
            'seo_optimization' => __('SEO Optimization', 'multi-author-plugin'),
            'headline_generation' => __('Headline/Title Generation', 'multi-author-plugin'),
            'outline_creation' => __('Outline Creation', 'multi-author-plugin'),
            'fact_checking' => __('Fact Checking Assistance', 'multi-author-plugin')
        );

        ?>
        <div class="map-ai-disclaimer-wrapper">
            <p class="description">
                <?php _e('Display an AI disclosure badge on this article to inform readers about AI usage.', 'multi-author-plugin'); ?>
            </p>

            <!-- Badge Type Selection -->
            <div class="map-ai-badge-type-section">
                <h4><?php _e('Badge Type', 'multi-author-plugin'); ?></h4>

                <label class="map-ai-badge-option">
                    <input type="radio" name="map_ai_badge_type" value="none" <?php checked($badge_type, 'none'); ?> />
                    <span class="map-ai-badge-label"><?php _e('No Badge', 'multi-author-plugin'); ?></span>
                    <span class="map-ai-badge-desc"><?php _e('Do not display any AI disclosure badge', 'multi-author-plugin'); ?></span>
                </label>
                ...
                <label class="map-ai-badge-option map-ai-badge-no-ai">
                    ...
                    <span class="map-ai-badge-label"><?php _e('No AI Used', 'multi-author-plugin'); ?></span>
                    <span class="map-ai-badge-desc"><?php _e('This article was created entirely without AI assistance', 'multi-author-plugin'); ?></span>
                </label>
                ...
                <label class="map-ai-badge-option map-ai-badge-enhanced">
                    ...
                    <span class="map-ai-badge-label"><?php _e('AI Enhanced', 'multi-author-plugin'); ?></span>
                    <span class="map-ai-badge-desc"><?php _e('AI tools were used in the creation of this article', 'multi-author-plugin'); ?></span>
                </label>
            </div>
            ...
            <div class="map-ai-uses-section" id="map-ai-uses-section" style="<?php echo $badge_type !== 'ai_enhanced' ? 'display: none;' : ''; ?>">
                <h4><?php _e('How was AI used?', 'multi-author-plugin'); ?></h4>
                <p class="description"><?php _e('Select all the ways AI was used in creating this article:', 'multi-author-plugin'); ?></p>
                ...
                <div class="map-ai-custom-uses">
                    <h4><?php _e('Custom AI Uses', 'multi-author-plugin'); ?></h4>
                    <p class="description"><?php _e('Add any additional ways AI was used that are not listed above:', 'multi-author-plugin'); ?></p>
                            ...
                            <?php foreach ($custom_uses as $index => $custom_use) : ?>
                                <div class="map-ai-custom-use-item">
                                    <input type="text"
                                           ...
                                           placeholder="<?php esc_attr_e('e.g., Voice cloning, 3D modeling', 'multi-author-plugin'); ?>"
                                           class="widefat" />
                                    <button type="button" class="button-link map-remove-custom-use" aria-label="<?php esc_attr_e('Remove', 'multi-author-plugin'); ?>">
                                        <span class="dashicons dashicons-no-alt"></span>
                                    </button>
                                </div>
                            <?php endforeach; ?>
                    ...
                    <button type="button" class="button" id="map-add-custom-ai-use">
                        <?php _e('+ Add Custom AI Use', 'multi-author-plugin'); ?>
                    </button>
                </div>
            </div>
        </div>
    ...
    public function save_meta_boxes($post_id, $post) {
        ...
        // Check post type
        if (!in_array($post->post_type, MAP_Settings::get_enabled_post_types(), true)) {
            return;
        }
        ...
        if (isset($_POST['map_ai_disclaimer_nonce_field']) &&
            ...
            if (isset($_POST['map_ai_uses']) && is_array($_POST['map_ai_uses'])) {
                ...
                $ai_uses = array_values(array_intersect($ai_uses, array_keys(MAP_Frontend_Display::get_ai_use_labels())));
            }
    ...
    public function ajax_search_users() {
        ...
        if (!current_user_can('edit_posts')) {
            wp_send_json_error(__('Permission denied.', 'multi-author-plugin'), 403);
        }
