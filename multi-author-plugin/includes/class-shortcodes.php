...
class MAP_Shortcodes {
    ...
    public function register_blocks() {
        ...
        wp_register_script(
            'map-blocks',
            MAP_PLUGIN_URL . 'admin/js/blocks.js',
    ...
    public function shortcode_contributors() {
        $html = MAP_Frontend_Display::get_instance()->render_contributors();
        return $html ? $html : $this->editor_placeholder(__('Contributor badges will appear here once contributors are assigned.', 'multi-author-plugin'));
    }
    ...
    public function shortcode_sources() {
        $html = MAP_Frontend_Display::get_instance()->render_sources();
        return $html ? $html : $this->editor_placeholder(__('The sources list will appear here once sources are added.', 'multi-author-plugin'));
    }
    ...
    public function shortcode_faq() {
        ...
        if (!$post_id || get_post_meta($post_id, '_map_faq_enabled', true) !== '1') {
            return $this->editor_placeholder(__('The FAQ section will appear here once enabled for this post.', 'multi-author-plugin'));
        }

        $html = MAP_FAQ::get_instance()->render_faq_section($post_id);
        return $html ? $html : $this->editor_placeholder(__('The FAQ section will appear here once questions are added.', 'multi-author-plugin'));
    }
    ...
    public function shortcode_corrections() {
        $html = MAP_Frontend_Display::get_instance()->render_corrections();
        return $html ? $html : $this->editor_placeholder(__('The corrections log will appear here once corrections are added.', 'multi-author-plugin'));
    }
    ...
    public function shortcode_editorial_team($atts = array()) {
        ...
        if (empty($team_members)) {
            return $this->editor_placeholder(__('No team members yet. Enable "Show on Editorial Team page" on user profiles, or pass IDs via the include attribute.', 'multi-author-plugin'));
        }

        $columns = min(4, max(1, intval($atts['columns'])));

        $template = MAP_Frontend_Display::locate_template('editorial-team.php');
