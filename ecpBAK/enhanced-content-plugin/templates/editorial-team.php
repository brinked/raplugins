 ...
 *
 * Available variables:
 *   $team_members - arrays from ECP_User_Profile::get_contributor_data()
...
$reviewed_label = apply_filters('map_team_reviewed_label', __('%s articles reviewed', 'enhanced-content-plugin'));
?>
...
<div class="map-editorial-team map-team-columns-<?php echo esc_attr($columns); ?>">
    <?php foreach ($team_members as $member) : ?>
        <div class="map-team-card">
            ...
            <?php
            $counts = ECP_Frontend_Display::get_credit_counts($member['id']);
            $credit_parts = array();
            if ($counts['reviewed']) {
                /* translators: %s: number of articles */
                $credit_parts[] = sprintf(_n('%s article reviewed', '%s articles reviewed', $counts['reviewed'], 'enhanced-content-plugin'), number_format_i18n($counts['reviewed']));
            }
            if ($counts['fact_checked']) {
                /* translators: %s: number of articles */
                $credit_parts[] = sprintf(_n('%s article fact-checked', '%s articles fact-checked', $counts['fact_checked'], 'enhanced-content-plugin'), number_format_i18n($counts['fact_checked']));
            }
