...
$skip_primary_author = ECP_Settings::get_setting('skip_primary_author', 0);
...
if (!$skip_primary_author && $primary_author) {
    $all_contributors[] = array(
        ...
        'user' => $primary_author,
        'role' => 'author',
        'role_label' => ECP_Settings::get_role_label('author'),
    ...
    foreach ($contributors['authors'] as $user_id) {
        ...
        if ($user) {
            $all_contributors[] = array(
                ...
                'user' => $user,
                'role' => 'coauthor',
                'role_label' => ECP_Settings::get_role_label('coauthor'),
    ...
    foreach ($contributors['reviewers'] as $user_id) {
        ...
        if ($user) {
            $all_contributors[] = array(
                ...
                'user' => $user,
                'role' => 'reviewer',
                'role_label' => ECP_Settings::get_role_label('reviewer'),
                'title' => get_user_meta($user_id, 'job_title', true)
            );
        }
    }
    ...
    foreach ($contributors['fact_checkers'] as $user_id) {
        ...
        if ($user) {
            $all_contributors[] = array(
                ...
                'user' => $user,
                'role' => 'fact_checker',
                'role_label' => ECP_Settings::get_role_label('fact_checker'),
                'title' => get_user_meta($user_id, 'job_title', true)
            );
        }
    }
...
$expert_verified_global = ECP_Settings::get_setting('expert_verified_enabled', 0);
...
$expert_verified_label = ECP_Settings::get_setting('expert_verified_label', __('Expert verified', 'enhanced-content-plugin'));
$expert_verified_title = ECP_Settings::get_setting('expert_verified_title', __('How is this page expert verified?', 'enhanced-content-plugin'));
$expert_verified_content = ECP_Settings::get_setting('expert_verified_content', '');
$expert_verified_link_text = ECP_Settings::get_setting('expert_verified_link_text', __('About our Review Board', 'enhanced-content-plugin'));
$expert_verified_link_url = ECP_Settings::get_setting('expert_verified_link_url', '');
...
function map_render_inline_popup($user_id, $role = 'author') {
    ...
    if ($role === 'author' || $role === 'coauthor') {
        ...
        if ($editorial_enabled && $show_for_authors) {
            $process_url = ECP_Settings::get_setting('editorial_process_url', '');
            $process_text = ECP_Settings::get_setting('editorial_process_text', __('Learn more about our editorial process', 'enhanced-content-plugin'));
        }
        ...
        if ($review_enabled && $show_for_reviewers) {
            $process_url = ECP_Settings::get_setting('review_process_url', '');
            $process_text = ECP_Settings::get_setting('review_process_text', __('Learn more about our review process', 'enhanced-content-plugin'));
        }
        ...
        if ($factcheck_enabled && $show_for_factcheckers) {
            $process_url = ECP_Settings::get_setting('factcheck_process_url', '');
            $process_text = ECP_Settings::get_setting('factcheck_process_text', __('Learn more about our fact-checking process', 'enhanced-content-plugin'));
        }
    }
    ...
    <div class="map-popup-content">
        ...
        <div class="map-popup-links">
            <a href="<?php echo esc_url($data['profile_url']); ?>" class="map-popup-link">
                <?php _e('View Full Bio', 'enhanced-content-plugin'); ?>
            </a>
        </div>
...
<div class="map-contributors-section">
    ...
    <div class="map-primary-row">
        ...
        ?>
        <span class="map-author-text"><?php esc_html_e('By', 'enhanced-content-plugin'); ?></span>
        ...
        <?php if ($mod_date) : ?>
            <span class="map-date-separator">|</span>
            <span class="map-date-text"><?php printf(esc_html__('Updated %s', 'enhanced-content-plugin'), esc_html($mod_date)); ?></span>
        <?php endif; ?>
        <?php endif; ?>
        <?php
        ...
        if ($fact_checked_date && preg_match('/^\d{4}-\d{2}-\d{2}$/', $fact_checked_date)) : ?>
            ...
            <span class="map-date-text map-fact-checked-date">
                ...
                <?php printf(esc_html__('Fact-checked %s', 'enhanced-content-plugin'), esc_html(date_i18n('F jS, Y', strtotime($fact_checked_date)))); ?>
            </span>
        <?php endif; ?>
        ...
        <?php if ($ai_badge_type === 'no_ai') : ?>
            <span class="map-ai-badge-wrapper">
                <span class="map-ai-badge map-ai-badge-no-ai" tabindex="0">
                    ...
                    <span class="map-ai-badge-text"><?php _e('No AI', 'enhanced-content-plugin'); ?></span>
                </span>
                <div class="map-ai-badge-popup">
                    <h4 class="map-ai-badge-popup-title"><?php _e('Human-Created Content', 'enhanced-content-plugin'); ?></h4>
                    ...
                    <?php if ($ai_disclaimer_url) : ?>
                        <a href="<?php echo esc_url($ai_disclaimer_url); ?>" class="map-ai-badge-popup-link" target="_blank" rel="noopener">
                            <?php _e('Learn more about our AI policy', 'enhanced-content-plugin'); ?>
                        </a>
                    <?php endif; ?>
                </div>
            </span>
        <?php elseif ($ai_badge_type === 'ai_enhanced') : ?>
            <span class="map-ai-badge-wrapper">
                ...
                <div class="map-ai-badge-popup">
                    <h4 class="map-ai-badge-popup-title"><?php _e('AI-Assisted Content', 'enhanced-content-plugin'); ?></h4>
                    <div class="map-ai-badge-popup-content">
                    ...
                    <?php if ($ai_disclaimer_url) : ?>
                        <a href="<?php echo esc_url($ai_disclaimer_url); ?>" class="map-ai-badge-popup-link" target="_blank" rel="noopener">
                            <?php _e('Read our full AI disclosure policy', 'enhanced-content-plugin'); ?>
                        </a>
                    <?php endif; ?>
                </div>
            </span>
    ...
    <?php if ($editorial_link) : ?>
        <div class="map-editorial-process-link">
            <a href="<?php echo esc_url($editorial_link); ?>"
               ...
               target="_blank"
               rel="noopener">
                <?php echo esc_html(ECP_Frontend_Display::get_editorial_process_label()); ?>
            </a>
        </div>
    <?php endif; ?>
</div>
